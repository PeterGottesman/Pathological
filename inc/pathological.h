...
class Pathological
{
  ...
  public:
	Pathological(unsigned w, unsigned h, unsigned spp, unsigned numthreads = std::thread::hardware_concurrency())
	    : rand(RandGen::rand_dev_seeded_generator(seed)), sc(), cam(sc, spp, rand), tex_width(w), tex_height(h),
	      nthreads(numthreads), spp(spp)
	{
