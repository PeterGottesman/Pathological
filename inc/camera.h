...
class Camera
{
  ...
  public:
	Camera(Scene& scene, unsigned spp, RandGen& rng) : rand(rng), scene(scene), spp(spp)
	{
	}

	Camera(Scene& scene, unsigned spp, Vec3 loc, Vec3 look, Vec3 up_dir, int w, int h, int fov, RandGen& rng)
	    : rand(rng), scene(scene), spp(spp), location(loc), look_point(look), up(up_dir)
	{
	...
	Color* get_image_buf(void) const
	{
		return image;
	}
	unsigned* get_iters_buf(void) const
	{
		return iters;
	}
	unsigned get_spp(void)
	{
		return spp;
	}
	...
	Color* make_image_buf(void* image_buf)
	{
		...
		image = (image_buf != nullptr) ? (Color*)image_buf : new Color[width * height];
	...
	void set_image_props(unsigned w, unsigned h, int fov, void* image_buf = nullptr)
	{
	...
	void look_at(const Vec3& point, const Vec3& up)
	{
		...
		this->x_axis = Vec3::normalize(Vec3::cross(this->dir, this->up));
